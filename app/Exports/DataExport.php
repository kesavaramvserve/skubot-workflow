...
class DataExport implements WithHeadings, WithMapping, FromCollection
{
    /**
    ...
    */
    ...
    function __construct($id,$title_sku1,$title_sku2,$title_sku3,$title_sku4,$title_sku5,
    ...
    $image_sku1,$image_sku2,$image_sku3,$image_sku4,$image_sku5,$category,$brand) {
        $this->id = $id;
    ...
    {
        $result = array_unique(array_merge($this->title_sku1,$this->title_sku2,$this->title_sku3,$this->title_sku4,$this->title_sku5,
        $this->description_sku1,$this->description_sku2,$this->description_sku3,$this->description_sku4,$this->description_sku5,
        $this->feature_sku1,$this->feature_sku2,$this->feature_sku3,$this->feature_sku4,$this->feature_sku5,
        $this->specification_sku1,$this->specification_sku2,$this->specification_sku3,$this->specification_sku4,$this->specification_sku5,
        $this->image_sku1,$this->image_sku2,$this->image_sku3,$this->image_sku4,$this->image_sku5));
        // dd($result);
        $query = WebsiteData::with('getFeature','getSpecification','getImage')->where('website_id',$this->id)->whereIn('id', $result);
        // $data = WebsiteData::with('getFeature','getSpecification','getImage')->where('website_id',$this->id)->whereIn('id', $result)->get();
        if (!blank($this->category)){
            $query = $query->where('category', $this->category);
        }
        if (!blank($this->brand)){
            $query = $query->where('brand', $this->brand);
        } 
        $data = $query->get();
        // dd($data);
        return $data;
