...
class QAController extends Controller
{
    ...
    {
        $qa_id = auth()->user()->id;
        $datas = WebsiteData::select('*','batch_id', DB::raw('count(*) as total'))->where('qa_done',0)->where('qa_id', $qa_id)->orderBy('total', 'desc')->groupBy('batch_id')->get();
        // dd($datas);
        return view('qa.index',compact('datas'));
    }
    ...
    {
        $qa_id = auth()->user()->id;
        $datas = WebsiteData::select('*','batch_id', DB::raw('count(*) as total'))->where('qa_id',$qa_id)->where('qa_done',1)->orderBy('total', 'desc')->groupBy('batch_id')->get();
        return view('qa.qa_complete',compact('datas'));
    }
}
